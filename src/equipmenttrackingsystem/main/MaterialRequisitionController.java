...
import static equipmenttrackingsystem.main.MaterialRequisitionController2.globalRequisition;
import static equipmenttrackingsystem.main.MaterialRequisitionController2.globalRequisitionDate;
import static equipmenttrackingsystem.main.MaterialRequisitionController2.globalRequisitionType;
 ...
 * @author CE
 */
public class MaterialRequisitionController implements Initializable {
    ...
    @FXML
    private AnchorPane frmMaterial;
    ...
    private void loadData() {
        ...
        try {
            arRequisition = MaterialRequisition.getAllRequisition();
    ...
    @FXML
    private void onSaveClicked(ActionEvent event) throws DataStorageException {
        ...
        if (!nullValue()) {
            ...
            if (!isMin()) {
                if (!isTextOnly()) {
                    ...
                    req = new MaterialRequisition(reqid, empid, reqname, reqdate, "Pending");
                    if (req.addNewRequisition()) {
                        globalRequisition = req.getLastAddedID();
                        globalRequisitionDate = dtRequisitionDate.getValue().toString();
                        globalRequisitionType = txtComment.getText() + "";
                        openRequisition();
                        AlertMessages.getInfo("Saved", "Requisition successfully saved");
                        ...
                        String screen = "/equipmenttrackingsystem/main/Requisition.fxml";
                        vehForm.openServiceListScreen(screen, title);
                    } else {
                        AlertMessages.getError("Unsuccessful", "Requisition was not saved");
                    }
    ...
    private void onContinueClicked(ActionEvent event) {
        if ("Cancelled".equals(cmbStatus.getValue())) {
            AlertMessages.getError("Unsuccessful", "Requisition has already been cancelled please select a pending requisition or create a new one");
        } else if ("Approved".equals(cmbStatus.getValue())) {
            AlertMessages.getError("Unsuccessful", "Requisition has already been approved please select a pending requisition or create a new one");
        } else if ("Complete".equals(cmbStatus.getValue())) {
            AlertMessages.getError("Unsuccessful", "Requisition has already been complete please select a pending requisition or create a new one");
        } else {
